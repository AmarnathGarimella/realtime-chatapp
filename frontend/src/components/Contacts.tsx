...
const Contacts = ({ contacts, currUser, changeChat }) => {
  ...
  const [currSelectedContact, setCurrSelectedContact] = useState(undefined);
  useEffect(() => {
    if (currUser) {
      setCurrUserName(currUser.username);
      setCurrUserImage(currUser.avtarImage);
    }
  }, [currUser]);

  const changeCurrChat = (index, contact) => {
    setCurrSelectedContact(index);
    changeChat(contact);
  };
  return (
    <>
            ...
            {contacts.map((contact, index) => {
              return (
                <div
                  key={index}
                  className={`contact ${
                    index === currSelectedContact ? "selected" : ""
                  }`}
